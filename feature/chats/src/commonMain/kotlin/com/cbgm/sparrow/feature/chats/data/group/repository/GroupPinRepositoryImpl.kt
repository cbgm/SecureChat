    ...
    override suspend fun pin(
        ...
        safeSuspendCall {
            ...
            updateMutex.withLock {
                ...
                val groupState = groupSecurityDao.findState(groupId) ?: error("Group security state was not found")
                val senderSigningPublicKey =
                    if (message.isMine) {
                        groupState.localSigningPublicKey.copyOf()
                    } else {
                        val senderContactId = requireNotNull(message.senderContactId) {
                            "Incoming group message has no sender contact"
                        }
                        groupSecurityDao
                            .findMemberKeys(groupId, groupState.currentEpoch)
                            .firstOrNull { member -> member.contactId == senderContactId }
                            ?.signingPublicKey
                            ?.copyOf()
                            ?: contactDao
                                .findById(senderContactId)
                                ?.publicIdentity
                                ?.signingPublicKey
                                ?.copyOf()
                ...
                dataSource.save(
                    createGroupPinEntity(
                        ...
                        messageSenderSigningPublicKey = senderSigningPublicKey,
